...
class Snake:
    ...
    def update_direction(self, key):
        if key in arrow_key_to_direction:
            ...
            valid_direction = len(self.parts) < 2 or (d[0] + self.parts[0][0], d[1] + self.parts[0][1]) != self.parts[1]
            if valid_direction:
                self.direction = d

    def move(self, grid):
        new_head = self.parts[0][0] + self.direction[0], self.parts[0][1] + self.direction[1]
        ...
        if len(self.parts) > self.length:  # trim tail of snake
            self.parts.pop()

        collides = self.parts[0] in self.parts[1:]
        if collides:
